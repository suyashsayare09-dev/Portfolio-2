...
(function initThree() {
  ...
  const renderer = new THREE.WebGLRenderer({ canvas, antialias: !isTouchDevice, alpha: true, powerPreference: 'high-performance' });
  ...
  /* Particles */
  const COUNT = window.innerWidth < 480 ? 40 : window.innerWidth < 768 ? 70 : window.innerWidth < 1100 ? 110 : 160;
  ...
  if (!prefersReducedMotion) buildLines();
  ...
  function animate() {
    ...
    if (!prefersReducedMotion && frameCount % (isTouchDevice ? 8 : 4) === 0) buildLines();
...
if (!prefersReducedMotion && tw) setTimeout(type, 1000);
else if (tw) tw.textContent = roles[0];

/* ── REVEAL ON SCROLL ───────────────────────────── */
const revealEls = document.querySelectorAll('.r-up');
const revObs = prefersReducedMotion ? null : new IntersectionObserver((entries) => {
  entries.forEach(e => {
    if (e.isIntersecting) { e.target.classList.add('vis'); revObs.unobserve(e.target); }
  });
}, { threshold: 0.12 });
revealEls.forEach(el => { if (revObs) revObs.observe(el); else el.classList.add('vis'); });
...
document.querySelectorAll('.tilt').forEach(card => {
  if (window.innerWidth < 992 || isTouchDevice) return;
...
window.addEventListener('resize', () => {
  if (window.innerWidth > 768 && mobileMenu.classList.contains('open')) toggleMenu(false);
}, { passive: true });
